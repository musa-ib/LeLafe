/**
 * @file 18_max.c
 * @brief Maximum Path Sum in Triangle
 * 
 * This program solves the problem of finding the maximum path sum in a triangle of numbers.
 * Starting from the top and moving to adjacent numbers in the row below, it calculates
 * the maximum possible sum along the path.
 * 
 * @details
 * Algorithm:
 * - Uses a bottom-up dynamic programming approach
 * - Starts from the second-last row and moves upward
 * - For each element, adds the maximum of its two possible children from the row below
 * - The final result is stored in the top element [0][0]
 * 
 * Complexity Analysis:
 * - Time Complexity: O(n²) where n is the number of rows
 * - Space Complexity: O(1) as we modify the input triangle in-place
 * 
...
/**
 * @brief Main function that solves the maximum path sum problem
 * @details Implements a bottom-up dynamic programming approach to find
 *          the maximum path sum in the triangle. The triangle is modified
 *          in-place, with each element being updated to store the maximum
 *          possible sum from that position to the bottom.
