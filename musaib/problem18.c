...
int main() {
    ...
    for (int i = 15 - 2; i >= 0; i--) { // Starting from 2nd last row
        for (int j = 0; j <= i; j++) {//Find the largest adjacent number in next row and add that to current elemnet
            pyramid[i][j] += (pyramid[i + 1][j] > pyramid[i + 1][j+1] ? pyramid[i + 1][j] : pyramid[i + 1][j+1]);
        }
    }
    printf("%d", pyramid[0][0]);
}
